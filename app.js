(() => {
  ...
  const state = { library: null, settings: null, activeTab: "library", selectedId: "root", playerVideoId: null, playerUrl: "", previewTitle: "", captions: { english: [], russian: [], studiedIds: [] }, player: null, youTubeReady: false, expanded: new Set(["root"]), layout: JSON.parse(localStorage.getItem("ytll-layout") || '{"mode":"columns","english":true,"russian":true}') };
  ...
  function renderFolderInspector(folder) {
    ...
    return `<header class="inspector-header"><div><p class="eyebrow">Папка</p><h1 class="view-title">${escapeHtml(folder.name)}</h1></div><span class="node-chip">${videos} роликов</span></header><div class="folder-summary"><b>Управление:</b> перетаскивайте папки и ролики внутри дерева. Правый клик по элементу открывает команды редактирования.</div><div class="form-actions"><button class="subtle-button" data-action="rename">Переименовать</button><button class="subtle-button" data-action="add-video">Добавить ролик</button><button class="subtle-button danger" data-action="delete">Удалить</button></div>`;
  }
  ...
  function renderPlayerV2() {
    ...
    const actions = id ? `<button class="mode-button" id="loadEnglish">ЗАГРУЗИТЬ EN</button><button class="mode-button ${translationReady ? "" : "needs-key"}" id="translateRussian">${translationReady ? "ПЕРЕВЕСТИ RU" : "КЛЮЧ OPENROUTER"}</button>` : "";
    return shell(`<section class="view player-view ${mode === "center" ? "center-mode" : ""}"><header class="player-head"><div><div class="player-title">${title}</div><div class="player-url">${url}</div></div><form class="player-link-form" id="playerLinkForm"><input name="url" value="${url}" placeholder="Вставьте YouTube-ссылку…" autocomplete="off" /><button class="subtle-button" type="submit">▶ Play</button><button class="primary" type="button" id="addRootVideo">＋ В библиотеку</button></form><div class="layout-actions">${actions}<button class="mode-button ${mode === "columns" ? "active" : ""}" data-mode="columns">КОЛОНКИ</button><button class="mode-button ${mode === "center" ? "active" : ""}" data-mode="center">ЦЕНТР</button><button class="mode-button" data-toggle-panel="english">EN ${state.layout.english ? "−" : "+"}</button><button class="mode-button" data-toggle-panel="russian">RU ${state.layout.russian ? "−" : "+"}</button></div></header><section class="learning-stage"><aside class="caption-panel ${state.layout.english ? "" : "collapsed"}"><header class="caption-head">English <span>${state.captions.english.length}</span></header><div class="caption-list">${captionsMarkup(state.captions.english, "en")}</div></aside><section class="video-zone">${player}<div class="center-captions"><b>${state.captions.english[0]?.text || "Субтитры по центру"}</b>${state.captions.russian[0]?.text || "Загрузите английскую дорожку и создайте перевод."}</div><nav class="study-controls" aria-label="Управление просмотром"><button class="control" data-player="back">↶ 5 сек</button><button class="control" data-player="play">▶ / ❚❚</button><button class="control" data-player="forward">5 сек ↷</button><button class="control" data-player="repeat">↻ реплика</button><span style="width:8px"></span>${["0.5","0.75","1","1.25","1.5","2"].map(rate => `<button class="control ${rate === "1" ? "active" : ""}" data-rate="${rate}">${rate}×</button>`).join("")}</nav></section><aside class="caption-panel ${state.layout.russian ? "" : "collapsed"}"><header class="caption-head">Русский <span>${state.captions.russian.length}</span></header><div class="caption-list">${captionsMarkup(state.captions.russian, "ru")}</div></aside></section></section>`);
  }
  function renderSettings() {
    const s = state.settings; const t = s.translation; const tr = s.transcription;
    return shell(`<section class="view settings-view"><form class="settings-card" id="settingsForm"><header class="settings-head"><p class="eyebrow">Настройки</p><h1 class="view-title">Локальная конфигурация</h1><p class="hint">Файлы настроек сохраняются рядом с переносным приложением.</p></header><section class="settings-group"><h2>Внешний вид</h2><label class="settings-row"><span><strong>Тема</strong><small>Светлая или тёмная оболочка.</small></span><select name="theme"><option value="dark" ${s.theme === "dark" ? "selected" : ""}>Тёмная</option><option value="light" ${s.theme === "light" ? "selected" : ""}>Светлая</option></select></label></section><section class="settings-group"><h2>OpenRouter</h2><label class="settings-row"><span><strong>API-ключ</strong><small>${t.hasApiKey ? "Ключ сохранён в зашифрованном виде. Оставьте пустым, чтобы не менять." : "Ключ ещё не сохранён."}</small></span><input name="apiKey" type="password" placeholder="sk-or-v1-…" /></label><label class="settings-row"><span><strong>Модель перевода</strong><small>Используется для пакетного перевода реплик.</small></span><input name="model" value="${escapeHtml(t.model)}" /></label></section><section class="settings-group"><h2>Локальная транскрибация</h2><label class="settings-row"><span><strong>Каталог моделей</strong><small>Уже скачанная модель faster-whisper.</small></span><input name="modelRoot" value="${escapeHtml(tr.modelRoot)}" /></label><label class="settings-row"><span><strong>Модель</strong><small>Используется только после согласия на локальную транскрибацию.</small></span><input name="whisperModel" value="${escapeHtml(tr.model)}" /></label><label class="settings-row"><span><strong>Команда uv</strong><small>Запускает независимый скрипт транскрибации.</small></span><input name="uvPath" value="${escapeHtml(tr.uvPath)}" /></label><label class="settings-row"><span><strong>Команда yt-dlp</strong><small>Временно скачивает аудио только с подтверждения пользователя.</small></span><input name="ytDlpPath" value="${escapeHtml(tr.ytDlpPath)}" /></label></section><footer class="settings-footer"><button type="button" class="subtle-button" id="resetSettings">Сбросить</button><button class="primary" type="submit">Сохранить настройки</button></footer></form></section>`);
  }
  ...
  function applyPlayerLayout() {
    ...
    stage.style.setProperty("--left-width", state.layout.english ? `${state.layout.leftWidth}px` : "0px");
    stage.style.setProperty("--right-width", state.layout.russian ? `${state.layout.rightWidth}px` : "0px");
  ...
  function mountPlayer() {
    ...
    state.player = null;
    stopCaptionSync();
    if (!host || !id || !state.youTubeReady || !window.YT?.Player) return;
  ...
  async function addUrlToRoot(url) {
    ...
    const title = await titleForUrl(cleanUrl);
    const video = { id: newId("video"), type: "video", name: title, url: cleanUrl, createdAt: new Date().toISOString(), progress: { studied: 0, position: 0 } };
  ...
  async function createFolder() { const name = prompt("Название папки:"); if (!name?.trim()) return; const folder = targetFolder(); folder.children ||= []; folder.children.push({ id: newId("folder"), type: "folder", name: name.trim(), children: [] }); await saveLibrary(); render(); }
  ...
  function bindEvents() {
    ...
    document.querySelectorAll("[data-mode]").forEach(button => button.addEventListener("click", () => { state.layout.mode = button.dataset.mode; persistLayout(); applyPlayerLayout(); }));
  ...
  window.onYouTubeIframeAPIReady = () => { state.youTubeReady = true; if (state.activeTab === "player") render(); };
